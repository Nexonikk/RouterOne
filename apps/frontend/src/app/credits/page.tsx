...
import { Coins, Plus, Loader2, CheckCircle2, AlertCircle, Wallet, TrendingUp } from "lucide-react";
...
function CreditsContent() {
  ...
  const totalCreditsUsed = apiKeys.reduce((sum: number, k: any) => sum + (k.credisConsumed ?? 0), 0);
      ...
      <div className="space-y-8">
            ...
            <CardContent>
              {apiKeysQuery.isLoading ? (
                ...
                <div className="space-y-2">
                  ...
                  {apiKeys.length > 4 && (
                    <p className="text-xs text-muted-foreground">+{apiKeys.length - 4} more</p>
                  )}
                </div>
              )}
            </CardContent>
        ...
        <Card className="bg-card/30 border-border/50">
          <CardHeader>
            ...
            <CardDescription>Top up your account with 1,000 credits per transaction.</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {onrampMutation.isError && (
              <div className="flex items-start gap-2.5 text-sm text-destructive bg-destructive/10 border border-destructive/20 rounded-lg px-3.5 py-3 mt-4">
                ...
                <span>{onrampMutation.error?.message || "Failed to add credits. Please try again."}</span>
              </div>
            )}
          </CardContent>
        </Card>
