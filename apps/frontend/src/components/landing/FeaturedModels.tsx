...
export default function FeaturedModels({ models }: { models: Model[] }) {
    const featuredModels = useMemo(() => {
        const seenCompanies = new Set<string>()

        return models
            .filter((model) => {
                if (seenCompanies.has(model.company.id)) {
                    return false
                }

                seenCompanies.add(model.company.id)
                return true
            })
            .slice(0, 3)
    }, [models])
                        ...
                        <motion.div key={model.name} variants={cardVariant}>
                            <TiltCard maxTilt={5}>
                                ...
                                >
                                    ...
                                    <p className="text-sm text-white/40">by {model.company.name}</p>
                                    {/* 
